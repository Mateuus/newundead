...
bool CUpdater::GetUpdaterData()
{
	...
	if(!downloadOk) {
		// if Download Failed then Download From Backup
		for(int retry = 0; retry < 5; retry++)
		{
			...
			data.clear();
			HttpDownload http;
			if(http.Get(gLauncherConfig.updateLauncherDataURL2.c_str(), data)) {
				downloadOk = true;
				break;
			}
			::Sleep(3000);
...
bool CUpdater::GetPackageData()
{
	...
	CkByteData   data;
	HttpDownload http(&prgTotal_);
	useGameDataURL2_ = false;
	if(!http.Get(gLauncherConfig.updateGameDataURL.c_str(), data)) {
		useGameDataURL2_ = true;
		if(!http.Get(gLauncherConfig.updateGameDataURL2.c_str(), data)) {
...
bool CUpdater::GetPackageData2()
{
	r3dOutToLog("Checking for game update 2\n");
	sprintf(updMsg1_, "Checking for game update 2");
	...
	sprintf(updMsg1_, "");
	r3dOutToLog("Checking for game update 2 - finished\n");
	return true;
}
...
void CUpdater::ValidateArchive()
{
	...
	if(!fswork_.ValidateArchive(false, prgTotal_.total, prgTotal_.cur))
	{
		...
		FailUpdate("Update succes, please restart your launcher.");
		r3d_assert(0);
	}
...
int CUpdater::GetNews()
{
	...
	if(!http.Get(gLauncherConfig.serverInfoURL.c_str(), xmlData)) {
		if(!http.Get(gLauncherConfig.serverInfoURL2.c_str(), xmlData)) {
			return 0;
		}
	}
...
void CUpdater::CheckSerialThreadEntry()
{
	try
	{
		...
		{
			sprintf(updMsg1_, "");
			// MessageBox(NULL, "Register at Godanel LuFy", "Serial Key Check", MB_OK);
			return;
		}
