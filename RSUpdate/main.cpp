...
static void CreateResources()
{
  ...
  g_imPB[0] = new r3dTexImage2D(IDR_IMAGE_PB_LEFT, 3, 4);
  g_imPB[1] = new r3dTexImage2D(IDR_IMAGE_PB_CENTER, 409, 4);
  g_imPB[2] = new r3dTexImage2D(IDR_IMAGE_PB_RIGHT, 3, 4);
  g_imBtnClose = new r3dTexImage2D(IDR_IMAGE_BTN_CLOSE_ON, 64, 64);
  g_imBtnLogin = new r3dTexImage2D(IDR_IMAGE_BTN_LOGIN_HOVER, 128, 64);
  g_imBtnRegister = new r3dTexImage2D(IDR_IMAGE_BTN_REGISTER_HOVER, 256, 64);
  g_imBtnRegisterNew = new r3dTexImage2D(IDR_IMAGE_BTN_REGISTER_NEW_HOVER, 256, 64);
  ...
  g_imBtnTopMenu[0] = new r3dTexImage2D(IDR_IMAGE_BTN_MYACC_HOVER, 256, 64);
  g_imBtnTopMenu[1] = new r3dTexImage2D(IDR_IMAGE_BTN_FORUMS_HOVER, 128, 64);
  g_imBtnTopMenu[2] = new r3dTexImage2D(IDR_IMAGE_BTN_SUPPORT_HOVER, 256, 64);
  g_imBtnTopMenu[3] = new r3dTexImage2D(IDR_IMAGE_BTN_YOUTUBE_HOVER, 32, 32);
  g_imBtnTopMenu[4] = new r3dTexImage2D(IDR_IMAGE_BTN_FACEBOOK_HOVER, 32, 32);
  g_imBtnTopMenu[5] = new r3dTexImage2D(IDR_IMAGE_BTN_TWITTER_HOVER, 32, 32);
  
  g_imBtnPg[0] = new r3dTexImage2D(IDR_IMAGE_BTN_PG_ON,    256, 128);
  g_imBtnPg[1] = new r3dTexImage2D(IDR_IMAGE_BTN_PG_HOVER, 256, 128);
...
static void drawProgressBar(CUpdater& updater)
{
  ...
  //g_font3->PrintF(655, 18, r3dColor(130, 130, 130), "version %s%s", UPDATER_VERSION,UPDATER_VERSION_SUFFIX);
  g_font3->PrintF(625, 18, r3dColor(130, 130, 130), "version %s %s", UPDATER_VERSION,UPDATER_VERSION_SUFFIX); // for Alpha Beta
  g_font2->DrawText(0, base_y-70, 785, 25, r3dColor(130, 130, 130), updater.updMsg1_, D3DFONT_CENTERED);
...
static void drawServerStatus(const CUpdater& updater)
{
  ...
  if(updater.newsStatus_ == 0) {
    g_font2->PrintF(595, y, r3dColor(130, 130, 130), "Retrieving Server Status");
    return;
  }

  if(updater.newsStatus_ == 1 || updater.serverStatus_.size() == 0) {
    g_font2->PrintF(607, y, r3dColor(130, 130, 130), "Server Status Unavailable");
    return;
  }
  
  g_font2->PrintF(610, y, r3dColor(130, 130, 130), "Server Status:");
  const char* status = updater.serverStatus_.c_str();
  if(stricmp(status, "ONLINE") == 0)
    g_font2->PrintF(700, y, r3dColor(31, 255, 57), "ONLINE");
  else if(stricmp(status, "OFFLINE") == 0)
    g_font2->PrintF(700, y, r3dColor(255, 31, 31), "OFFLINE");
  else
    g_font2->PrintF(700, y, r3dColor(255, 127, 39), status);
...
{
	 char p[512];
	 sprintf(p,"\"%s\" %s",GAME_EXE_NAME,param);
    BOOL res = CreateProcess(
    NULL,
    p,
    NULL,
    NULL,
    FALSE,
    CREATE_PROTECTED_PROCESS, // no console by default, if needed game server will alloc it
    NULL,
    NULL,
    NULL,
    NULL);
...
*/  
  ...
  */
  ...
  //startGame(GAME_START_PARAM);
  //CreateProcessByName("MiniA.exe");
  return;
}
