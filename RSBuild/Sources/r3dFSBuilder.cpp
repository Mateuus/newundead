...
bool r3dFSBuilder::ShouldExclude(const char* full, const char* name, int isDir)
{
  ...
  for(size_t i=0; i<excludes_.size(); i++)
  {
    ...
    {
      if(pattern_match(full, match.name)) {
		 // r3dOutToLog("!!! Ignored %s because of %s\n", full, match.name);
    ...
    {
      if(pattern_match(name, match.name)) {
		 //  r3dOutToLog("!!! Partially ignored %s because of %s\n", full, match.name);
...
DWORD r3dFSBuilder::DetectFileFlags(const char* fname)
{
  ...
  if(pattern_match(fname, "*.exe")			     || 
	 ...
	 pattern_match(fname, "HID.dll")			 || 
	 pattern_match(fname, "GayGuard.dll")		 || 
	 pattern_match(fname, "WarGuard.dll")		 || 
	 pattern_match(fname, "WarGuard\*.*")		 || 
	 pattern_match(fname, "ts3client_win32.dll") || 
     pattern_match(fname, "ts3server_win32.dll") || 
	 pattern_match(fname, "HackShield\*.*")	   	 || 
	 pattern_match(fname, "Music\*.*")			 || 
	 pattern_match(fname, "*.jpg")				 ||
	 ...
	 pattern_match(fname, "AH.dat")			     || 
     pattern_match(fname, "*.manifest")			 ||
   //pattern_match(fname, "splash.bmp")			 || 
