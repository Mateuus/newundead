...
extern BOOL DataCompare( BYTE* pData, BYTE* bMask, char* szMask );
//extern DWORD FindPattern( DWORD dwAddress, DWORD dwLen, BYTE* bMask, char* szMask );
extern DWORD FindPattern( HMODULE hModule, DWORD dwLen, BYTE* bMask, char* szMask, DWORD offset = 0 );
extern DWORD FindPattern(char *module, char *pattern, char *mask);
extern void r3dOutToConsole(const char* Str, ...);
extern bool r3dHaveString(const char* src, const char* str);
extern bool r3dAddReport(const char* Str, ...);
...
//r3dSec_type<r3dRenderLayer, 0xAC8DA94A> r3dRenderer;
...
extern	int			r3dUpdateScreenShot(bool getcheat = false,IDirect3DSurface9** ss = NULL);
extern	IDirect3DSurface9*			r3dGetCheatScreenShot();
...
extern_nspace(game, void	WarGuardWorkers(int flags));
extern_nspace(game, void	MainLoop());
extern_nspace(game, void	Shutdown());
extern_nspace(game, void	InitVoIP());
...
//extern_nspace(win,  long HsCallBack);
...
struct r3dLevelTerrain
{
	char data[8192*2];
};

struct r3dLevelInfos
{
	char roommat[1024*128];
	char setting[1024*64];
	char editdata[1012*32];
};

struct r3dLevelTerrainHeader
{
	long len;
	char data[8192*2];
};

struct r3dLevelSettings
{
	char* data1;
	char* data2;
};
...
{
	return (0 == ((_FPCLASS_SNAN | _FPCLASS_QNAN | _FPCLASS_NINF | _FPCLASS_PINF | _FPCLASS_ND | _FPCLASS_PD) & _fpclass(f) ));
}
