...
typedef std::vector<ColorStringList> clrstringlist_t;
...
bool	imgui_DirList(float sx, float sy, float sw, float sh, const char *Dir, const char *Extensions, char *edit_val, float *pDirFilesOffset, int *pCurSel, bool bUseDesktop = true, bool bUseCheck = false, bool bChangedVal= false, int* pDragIndex = NULL );
