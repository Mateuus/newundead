...
REG_VAR( g_password		, ""		, VF_SAVE ); // Password All right Seting
...
REG_VAR( r_voip   , true   , VF_SAVE );  // fullscreen mode
REG_VAR( r_voipPlayBack   , 0   , VF_SAVE );
REG_VAR( r_voipCapture   , 0   , VF_SAVE );
...
REG_VAR( g_serverip,			"167.114.32.63",	0 ); // 66.180.197.56 - IP of dev server where we host dev master\supervisor
REG_VAR( g_api_ip,				"167.114.32.63", 0);
