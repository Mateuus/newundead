...
static void imgui2_FillFiles(const char* Dir)
{
  ...
  if(h != INVALID_HANDLE_VALUE) {
    do {
      if (ffblk.cFileName[0] != '.') imgui2_DirFiles.push_back(ffblk.cFileName);
...
bool imgui2_FileList(float sx, float sy, float sw, float sh, const char* Dir, char *edit_val, bool bUseDesktop)
{
  imgui2_FillFiles(Dir);

  static int CurSel = -1;
  int idx = -1;
  if(imgui2_DrawList(sx, sy, sw, sh, imgui2_DirFiles, CurSel, &imgui2_DirFilesOffset, &idx, bUseDesktop) == false) 
    return 0;
