...
int r3dNetwork::Initialize(r3dNetCallback* callback, const char* networkName)
{
  ...
  //#ifdef WO_SERVER
  //impl->peer->SetTimeoutTime(1000*20,RakNet::UNASSIGNED_SYSTEM_ADDRESS);
  //#endif
  ...
  while((p = impl->peer->Receive()) != NULL)
  {
   unsigned char* data = p->data;
	...
	impl->stat_.packetlossTotal = (impl->stat_.packetlossTotal + rns->packetlossTotal) / 2.0f;
      ...
      default:
      {
		 // r3dOutToLog("PKT R %d\n",((const r3dNetPacketHeader*)data)->EId);
        if(/*data[0]*/((const r3dNetPacketHeader*)data)->EId < FIRST_FREE_PACKET_ID)
        {
...
//#endif
  ...
  /*if (r3dGetTime() > Time+1)
  {
	  r3dOutToLog("r3dNetwork SendData %d\n",CountofSend);
	  CountofSend = 0;
	  Time = r3dGetTime();
  }*/
...
int r3dNetwork::CreateHost(int port, int numPeers)
{
  ...
  #ifdef WO_SERVER
  impl->peer->SetTimeoutTime(15000, RakNet::UNASSIGNED_SYSTEM_ADDRESS);
  #endif
...
int r3dNetwork::CreateClient()
{
 ...
 #ifdef WO_SERVER
  impl->peer->SetTimeoutTime(50000, RakNet::UNASSIGNED_SYSTEM_ADDRESS);
#endif
...
void r3dNetwork::SendToHost(const r3dNetPacketHeader* data, int dataSize, bool isReliable)
{
  ...
  */  
  ...
  //PacketPriority priority = isReliable ? HIGH_PRIORITY : MEDIUM_PRIORITY;
   //data->EId = data->EventID;

  PacketReliability reliability = isReliable ? RELIABLE_ORDERED : UNRELIABLE_SEQUENCED;
  impl->peer->Send((const char*)data, dataSize, IMMEDIATE_PRIORITY, RELIABLE_ORDERED, 0, impl->peerData[0], false);
   
  //r3dOutToLog("r3dNetwork: send to host, len:%d, %08x, %08x\n", dataSize, ((DWORD*)data)[0], ((DWORD*)data)[1]);
//	  r3dOutToLog("r3dNetwork: send to host, eventid %d\n",data->EventID);
  return;  
}
...
void r3dNetwork::SendToPeer(const r3dNetPacketHeader* data, int dataSize, DWORD peerId, bool isReliable)
{
 ...
 // r3d_assert(data->EventID >= r3dNetwork::FIRST_FREE_PACKET_ID);
  ...
  //PacketPriority priority = isReliable ? IMMEDIATE_PRIORITY : MEDIUM_PRIORITY;

  //PacketReliability reliability = isReliable ? RELIABLE_ORDERED : UNRELIABLE_SEQUENCED;
  impl->peer->Send((const char*)data, dataSize, IMMEDIATE_PRIORITY, RELIABLE_ORDERED, 0, impl->peerData[peerId], false);
 // r3dOutToLog("SendToPeer : EventID %d to peer %d\n",data->EventID,peerId);
  CountofSend++;
  return;
}
