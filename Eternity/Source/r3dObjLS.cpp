...
bool r3dMesh::DoLoad( bool use_default_material )
{
    ...
    sprintf(fname1,"%s.scb",fname1);
	fname = (const char*)fname1;
	FileName = fname;*/
	//r3dOutToLog("FileName %s\n",FileName.c_str());

	bool bin_exist = r3d_access(bin_file, 0) == 0;
	bool txt_exist = r3d_access(FileName.c_str(), 0) == 0;
...
//#endif
	...
	if(bin_exist && !txt_exist) // if bin exists but text sco is no longer there, show error and bail out
	{
		...
		r3dArtBug("r3dMesh::Load(): Missing sco file for %s change to load sco.\n", fname);
	}
#endif
#ifdef FINAL_BUILD
	if (bin_exist && !txt_exist)
	{
		std::string str= FileName.c_str();
		int pos = str.find(".");
		std::string str2 = str.substr(0,pos);
		char fname2[512] = {0};
		sprintf(fname2,"%s.scb",str2.c_str());
		//sprintf(FileName.c_str(),fname);
		FileName = fname2;
		fname = fname2;
		//r3dOutToLog(".sco to .scb %s\n",fname);
	}
#endif
