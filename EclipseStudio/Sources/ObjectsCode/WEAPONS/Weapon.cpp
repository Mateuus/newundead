...
void Weapon::Reset()
{
	...
	m_lastReloadingTime = 0;
	m_State = WPN_EMPTY;
	/*if(getNumBulletsLeft() > 0 && getReloadProgress() < m_ReloadTime)
	  m_State = WPN_RELOADING;
	else*/ if (getNumBulletsLeft() > 0)
       m_State = WPN_READY;
	...
	if(m_pConfig->m_itemID == 101306) // hard coded for melee flashlight
	{
		...
		m_Flashlight.SetRadius(10.5f, 70.0f);
...
float Weapon::getRateOfFire() const
{
	...
	
	{
       ...
       // r3dOutToLog("nRetWeapons %d\n",nRet);
		...
		//r3dOutToLog("Firedelay itemid '%d' , '%d'\n",m_pConfig->m_itemID,fireDelay);
	}
...
void Weapon::Update(const D3DXMATRIX& weaponBone)
{
 ...
 obj_Player* plr = gClientLogic().localPlayer_;
     if (m_flashlightToggle && !m_Flashlight.IsOn())	{
...
{ 
	...
	const wiCharDataFull& slot = gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID];
	// NOTE : NEED TO SYNC WITH SERVER!
	if (slot.Stats.skillid11) // decreased spread 20%
    recoil *= 0.8f;
...
void Weapon::Reload()
{
	...
	if (getPlayerItem().itemID == 101087 ||
   getPlayerItem().itemID == 101085 ||
    getPlayerItem().itemID == 101084 ||
	getPlayerItem().itemID == 101068 ||
   getPlayerItem().itemID == 101088 ||
  getPlayerItem().itemID == 101217 ||
	getPlayerItem().itemID == 101232 ||
	getPlayerItem().itemID == 101247)
	{

		if (getPlayerItem().Var1 > 20)
		{
			r3dOutToLog("Hack Bullet detected!!\n");
			// Sent Chect Packet to server

		}

	}

	//r3dOutToLog("WeaponItemID : '%d'\n",getPlayerItem().itemID);
