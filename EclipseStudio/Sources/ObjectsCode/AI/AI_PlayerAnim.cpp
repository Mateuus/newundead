...
void CUberData::LoadLowerAnimations()
{
	...
	i[4] = AddAnimation("Swim_Fast_Left");
	i[5] = AddAnimation("Swim_Fast_Right");
...
void CUberData::LoadAnimations()
{
	...
	aid_.UI_IdleNoWeapon = AddAnimation("Idle_Hands_01");
...
float CUberAnim::GetGrenadeLaunchFrame()
{
	...
	const static float GrenadeLaunchFrames[][2] = 
	{
		 ...
		 {0, 0}, //PLAYER_SWIM,
		 {0,0}, //PLAYER_SWIM_LEFT,
		 {0, 0}, //PLAYER_SWIM_RIGHT,
...
int id;
	id = anim.StartAnimation(data_->aid_.hands[0],0, 0.0f, 1.0f, 0.1f);
anim.GetTrack(id)->fSpeed = 1.0f;
	// resync animation, so jump track will be relocated to top of lower bodys anim
