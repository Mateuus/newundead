...
void obj_AnimalSpawn::AppendRenderables(RenderArray (& render_arrays  )[ rsCount ], const r3dCamera& Cam)
{
#ifdef FINAL_BUILD
	return;
#else
...
#ifndef FINAL_BUILD
float obj_AnimalSpawn::DrawPropertyEditor(float scrx, float scry, float scrw, float scrh, const AClass* startClass, const GameObjects& selected)
{
	float y = scry;
	...
	y += imgui_Value_SliderI(scrx, y, "Max Animals", &zc, 0, 100.0f, "%d");
	maxZombieCount = zc;
	...
	y += imgui_Value_Slider(scrx, y, "%% FastAnimals", &fastZombieChance, 0.0f, 100.0f, "%-02.3f");
	y += imgui_Value_Slider(scrx, y, "%% Speed Variation", &speedVariation, 0.0f, 50.0f, "%-02.3f");

	y += 5.0f;
	y += imgui_Static(scrx, y, "Animal senses parameters:");
	...
	y += 5.0f;
	y += imgui_Static(scrx, y, "Deer's to spawn:");
	// Animal selection
	{
		...
		if(numZombies == 0)
		{
			...
			while(g_pWeaponArmory->searchNextItem())
			{
				...
				if( cfg && cfg->m_Weight < -1.0f ) // weight -1 is for zombies only
				{
					tempS holder;
...
void obj_AnimalSpawn::WriteSerializedData(pugi::xml_node& node)
{
	...
	SetXMLVal("max_animals", zombieSpawnNode, &mzc);
	//SetXMLVal("sleepers_rate", zombieSpawnNode, &sleepersRate);
	SetXMLVal("animals_spawn_delay", zombieSpawnNode, &zombieSpawnDelay);
	...
	SetXMLVal("fastAnimalChance", zombieSpawnNode, &fastZombieChance);
	SetXMLVal("speedVariation", zombieSpawnNode, &speedVariation);
	zombieSpawnNode.append_attribute("numAnimalSelected") = ZombieSpawnSelection.size();
