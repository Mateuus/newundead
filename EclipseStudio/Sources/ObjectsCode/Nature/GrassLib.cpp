...
GrassLib::Load()
{
	...
	if( strcmp( CheckSig, Graz_SIG )!=0 )
	{
		if( strcmp( CheckSig, Graz_SIG2 )!=0 )
		{
			if( strcmp( CheckSig, Graz_SIG3 ) != 0 )
			{
	...
	{
		fseek( fin, (int)sizeof Graz_SIG - (int)sizeof Graz_SIG2, SEEK_CUR );
	}
	...
	for( uint32_t i = 0, e = mEntries.Count(); i < e; i ++ )
	{
		...
		{
			//	No lazy loading. Perform entry loading here.
			en.fileStartOffset = ftell(fin);
			if (!LoadGrassLibEntry(en, fin))
				return false;
		}
	}
