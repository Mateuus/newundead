...
BOOL obj_ParticleSystem::Load(const char* fname)
{
	...
	sprintf(Str1,"Data\\Particles\\%s.prt", fname);
...
/*virtual*/
void
obj_ParticleSystem::AppendShadowRenderables( RenderArray & rarr, const r3dCamera& Cam ) /*OVERRIDE*/
{
#ifndef FINAL_BUILD
	if( !r_draw_particles->GetInt() )
		return ;
...
#endif
	{
		...
		if(effectQuality > (int)r_particles_quality->GetMaxVal()) effectQuality = (int)r_particles_quality->GetMaxVal();
		if(effectQuality > r_particles_quality->GetInt())
			return;
	}
