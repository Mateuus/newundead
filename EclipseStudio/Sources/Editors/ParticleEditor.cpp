...
void ParticleReloadMesh(r3dParticleEmitter &PE, const char *Str2)
{
	SAFE_DELETE(PE.Mesh);
	PE.Mesh = new r3dMesh( 0 );
	PE.Mesh->Load(Str2);
	PE.Mesh->FillBuffers();
}
...
static void DrawGeneralParams(int SlotID)
{
	...
	imgui2_Value_Old(1.0f,60.0f, "%.4f", "Frame Rate", &PE.FrameRate,false);
...
static void DrawTypeParams(int SlotID)
{
	...
	case R3D_PARTICLE_MESH:
		...
		if(imgui2_StringValue("Mesh", buf, false)) {
			PE.MeshFName = buf;
			EnableListPick(PICK_MeshName);
		}
		imgui2_Checkbox("disable culling", &PE.bMeshDisableCulling);

		strcpy_s(buf, _countof(buf), PE.MeshTexFName.c_str());
		if(imgui2_StringValue("Texture", buf, false)) {
			PE.MeshTexFName = buf;
			EnableListPick(PICK_MeshTexture);
		}

		imgui2_Checkbox("Use Distort", &PE.bMeshUseDistortTexture);

		if ( PE.bMeshUseDistortTexture )
		{
			strcpy_s(buf, _countof(buf), PE.MeshDistortTexFName.c_str());
			if(imgui2_StringValue("DistortTexture", buf, false)) {
				PE.MeshDistortTexFName = buf;
				EnableListPick(PICK_MeshDistortTexture);
			}
		}
...
static void DrawParticleSlot(int SlotID)
{
	...
	if(EditTorch->PType[SlotID] == NULL) 
	{
		if(imgui2_ButtonEx(z.X+20, z.Y+25, 60, 20, "NEW")) {
			...
			EditTorch->PType[SlotID] = new r3dParticleEmitter;
			r3dParticleEmitter* pe = EditTorch->PType[SlotID];
	...
	case eReloadTex: // reload tex
		{
			...
			if(EditTorch)
			{
				sprintf(Str2, PARTICLE_DATA_PATH_PATTERN, EditTorch->TextureFileName.c_str());
				if(EditTorch->Texture)
					r3dRenderer->DeleteTexture(EditTorch->Texture,1);

				EditTorch->Texture = r3dRenderer->LoadTexture(Str2);
			}
			break;
		}
	case eReset:
...
static bool DrawPickMenu()
{
	...
	case PICK_MeshTexture:
		{
			r3dParticleEmitter& PE = *EditTorch->PType[curEmitterSlot];
			PE.MeshTexFName = outName;

			ParticleReloadTex(&PE.MeshTexture, Str2);
			break;
		}
	...
	case PICK_MeshDistortTexture:
		{
			r3dParticleEmitter& PE = *EditTorch->PType[curEmitterSlot];
			PE.MeshDistortTexFName = outName;

			ParticleReloadTex(&PE.MeshDistortTexture, Str2);
			break;
		}
