...
int LoadXMLSettings()
{
	...
	char fn[512] = {0};
	sprintf(fn,"%s\\LevelInfos\\LevelInfos.bin",r3dGameLevel::GetHomeDir());
#ifndef FINAL_BUILD
	f = r3d_open( fn, "rb" );
	...
	if (f)
	{
		...
		XMLExists = true ;
		r3dLevelInfos data;
		ZeroMemory(&data,sizeof(r3dLevelInfos));
		fread(&data, sizeof(r3dLevelInfos), 1, f );
		fclose(f);
		pugi::xml_parse_result parseResult = xmlLevelFile.load_buffer_inplace(data.setting, strlen(data.setting));
	    if(!parseResult)
		{
...
int LoadLevel( float startLoadingProgress )
{
	...
	windSts.DirX = -0.441881f;
	windSts.DirZ = 0.897074f;
	windSts.WindPatternScale = 1369.59f;
	windSts.WindPatternSpeed = 1660.15f;
...
void SaveLevelData( char* Str )
{
	...
	xmlLevel.append_attribute("version2") = 100;
	...
	/*{
		FILE* f = fopen(Str,"rb");
		r3dLevelInfos data;
		fread(&data.dataheader,1,512,f);
		fclose(f);
		sprintf(Str, "%s\\room.mat", r3dGameLevel::GetSaveDir());
		f = fopen(Str,"rb");
		fread(&data.room,1,1024*1024,f);
		fclose(f);
		sprintf(Str, "%s\\LevelSettings.xml", r3dGameLevel::GetSaveDir());
        f = fopen(Str,"rb");
		fread(&data.Settings,1,8192*4,f);
		fclose(f);
		sprintf(Str, "%s\\LevelInfos.bin", r3dGameLevel::GetSaveDir());
		f = fopen(Str,"wb");
		fwrite(&data,sizeof(r3dLevelInfos) - ((1024*1024) - strlen(data.room)),1,f);
		fclose(f);
		ZeroMemory(&data,sizeof(r3dLevelInfos));
	}*/

	sprintf(Str,"%s\\LevelInfos",r3dGameLevel::GetSaveDir());
	_mkdir(Str);

	sprintf(Str, "%s\\LevelInfos\\LevelInfos.bin", r3dGameLevel::GetSaveDir());
	FILE* bin = fopen_for_write( Str, "wb");
	r3dLevelInfos datas;
	ZeroMemory(&datas,sizeof(r3dLevelInfos));
	{
	sprintf(Str, "%s\\LevelSettings.xml", r3dGameLevel::GetSaveDir());
	r3dFile* file = r3d_open(Str,"rb");
	//char* setting = new char[file->size + 1];
	fread(&datas.setting,file->size,1,file);
	fclose(file);
	/*int size = file->size;
	fwrite(&size,sizeof(int),1,bin);
	fwrite(setting,file->size,1,bin);
	fclose(file);*/
	}

	{
	sprintf(Str, "%s\\EditorSettings.xml", r3dGameLevel::GetSaveDir());
	r3dFile* file = r3d_open(Str,"rb");
	//char* setting = new char[file->size + 1];
	fread(&datas.editdata,file->size,1,file);
	fclose(file);
	}

	fwrite(&datas,sizeof(r3dLevelInfos),1,bin);
	...
	fread(data,file->size,1,file);
	fclose(file);
	
	{
	sprintf(Str, "%s\\LevelInfos\\LevelsInfos_0.bin", r3dGameLevel::GetSaveDir());
	...
	/*{
	sprintf(Str, "%s\\Terrain2\\Mat-Splat0.dds", r3dGameLevel::GetSaveDir());
	r3dFile* file = r3d_open(Str,"rb");
	if (file)
	{
	void* data = NULL;
	...
	//pugi::xml_node xmlObject = xmlLevel.child("object");
	//int count = 0;
	//char data[1024];
	__int64 i = 0;
	__int64 i2 = 0;
	...
	//int i2 = 0;
	char dir[512] = {0};
	sprintf(dir,"%s\\LevelData",r3dGameLevel::GetSaveDir());
	_mkdir(dir);
	typedef std::map< std::string, int > MissingMap;
	MissingMap missingMeshes;
	...
	for(ObjectIterator iter = GameWorld().GetFirstOfAllObjects(); iter.current; iter = next )
	{
		...
		{
			if (obj->bPersistent )
				if(obj->isSerializable() && obj->Class->Name != "obj_AmbientSound" && obj->Class->Name != "obj_LightHelper")
				{
					char name[128] = {0};
					sprintf(name,"%s\\LevelData\\ObjectData.bin", r3dGameLevel::GetSaveDir());
					char name2[128] = {0};
					sprintf(name2,"%s\\LevelData\\ObjectData.xml", r3dGameLevel::GetSaveDir());
					pugi::xml_document xmlLevelFile1;
					xmlLevelFile1.append_child(pugi::node_comment).set_value("Object data");
					pugi::xml_node xmlLevel1 = xmlLevelFile1.append_child();
					xmlLevel1.set_name("ObjectData");
					SaveLevelObject(xmlLevel1,obj);
					xmlLevelFile1.save_file(name2);

					FILE * hFile;
					FILE * hFile2;
					if (r3d_access(name,0))
					hFile = fopen_for_write( name, "wb");
					else
					hFile = fopen_for_write( name, "r+");

					r3dLevelData data;
					//data.xmlObject = xmlObject;
					sprintf(data.filename,obj->FileName.c_str());
					sprintf(data.class_name,obj->Class->Name.c_str());

					//pugi::xml_node posNode = xmlObject.child("position");
					//r3dPoint3D pos(0, 0, 0);
					data.pos = obj->GetPosition();

					hFile2 = fopen( name2, "rb");

					//char* txt = new char[8192];
					fread(&data.text,8192,1,hFile2);
            
					//sprintf(data.text,txt);
					fwrite(&data, sizeof(r3dLevelData)-(8192-strlen(data.text)), 1, hFile );
				...
				else if (obj->Class->Name == "obj_AmbientSound")
				{

					char name[128] = {0};
					sprintf(name,"%s\\LevelData\\SoundData.bin", r3dGameLevel::GetSaveDir());
					char name2[128] = {0};
					sprintf(name2,"%s\\LevelData\\SoundData.xml", r3dGameLevel::GetSaveDir());
					pugi::xml_document xmlLevelFile1;
					xmlLevelFile1.append_child(pugi::node_comment).set_value("Sound data");
					pugi::xml_node xmlLevel1 = xmlLevelFile1.append_child();
					xmlLevel1.set_name("ObjectData");
					SaveLevelObject(xmlLevel1,obj);
					xmlLevelFile1.save_file(name2);
				...
				else if (obj->Class->Name == "obj_LightHelper")
				{

					char name[128] = {0};
					sprintf(name,"%s\\LevelData\\LightData.bin", r3dGameLevel::GetSaveDir());
					char name2[128] = {0};
					sprintf(name2,"%s\\LevelData\\LightData.xml", r3dGameLevel::GetSaveDir());
					pugi::xml_document xmlLevelFile1;
					xmlLevelFile1.append_child(pugi::node_comment).set_value("Light data");
					pugi::xml_node xmlLevel1 = xmlLevelFile1.append_child();
					xmlLevel1.set_name("ObjectData");
					SaveLevelObject(xmlLevel1,obj);
					xmlLevelFile1.save_file(name2);
		...
		//fwrite("sadasdasdasdasdsadasd", strlen("sadasdasdasdasdsadasd"), 1, hFile );
		//fwrite("dsfdsfdsfsdfdsfdsfds", strlen("dsfdsfdsfsdfdsfdsfds"), 1, hFile );
	}
}
...
void Editor_Level :: PackTerrain()
{
	char name[512];
	sprintf(name,"%s\\Terrain2\\terrain2.bin",r3dGameLevel::GetHomeDir());
	...
	fclose(f);
	sprintf(name,"%s\\Terrain2\\terrain2.ini",r3dGameLevel::GetHomeDir());
	f = fopen(name,"rb");
	char buf[8192*2] = {0};
	fread(&buf,8192*2,1,f);
	fclose(f);
	//ZeroMemory(&data,sizeof(r3dLevelTerrain));
	sprintf(name,"%s\\Terrain2\\Terrain2Header.bin",r3dGameLevel::GetHomeDir());
	r3dLevelTerrainHeader data;
	f = fopen(name,"wb");
	data.len = size;
	sprintf(data.data,buf);
	fwrite(&data,sizeof(r3dLevelTerrainHeader)- (8192*2 - strlen(data.data)),1,f);
	ZeroMemory(&data,sizeof(r3dLevelTerrainHeader));
	ZeroMemory(&buf,8192*2);
	fclose(f);
...
void Editor_Level :: ProcessSettings()
{
	...
	if ( imgui_Button( 215, 85, 100, 35, "PACK TERRAIN", 0, false ) ) 
	{
            const float WI = 220.f ;
			if (!Terrain2)
			imgui_Static( r3dRenderer->ScreenW2 - WI * 0.5f, r3dRenderer->ScreenH2, "Not have terrain2.", WI, false, 22, false ) ;
			else
			{
            PackTerrain();
			imgui_Static( r3dRenderer->ScreenW2 - WI * 0.5f, r3dRenderer->ScreenH2, "Packed!", WI, false, 22, false ) ;
			}
	}
...
void FillCatFromDir( CategoryStruct& cat, const char* clazz, const char* path, const char *filePattern = "%s*.sc*")
{
	char CatPath[ 256 ];
	char TempS[ 256 ];
