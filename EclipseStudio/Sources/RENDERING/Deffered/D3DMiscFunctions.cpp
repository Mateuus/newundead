...
void UpdateD3DAntiCheat_WallHack()
{
	...
	//	if( r_debug_helper->GetInt() )
		...
		D3D_V( r3dRenderer->pd3ddev->SetRenderState(D3DRS_FOGENABLE, FALSE));
		D3D_V( r3dRenderer->pd3ddev->SetRenderState(D3DRS_LIGHTING, FALSE));
		D3D_V( r3dRenderer->pd3ddev->SetRenderState( D3DRS_COLORWRITEENABLE, D3DCOLORWRITEENABLE_RED ) );
		D3D_V( r3dRenderer->pd3ddev->SetRenderState( D3DRS_COLORWRITEENABLE1, D3DCOLORWRITEENABLE_GREEN ) );
		D3D_V( r3dRenderer->pd3ddev->SetRenderState( D3DRS_COLORWRITEENABLE2, D3DCOLORWRITEENABLE_BLUE ) );
		D3D_V( r3dRenderer->pd3ddev->SetRenderState( D3DRS_COLORWRITEENABLE3, D3DCOLORWRITEENABLE_ALPHA ) );

		//D3D_V(r3dRenderer->pd3ddev->SetRenderState(D3DRS_AMBIENT,D3DCOLOR_ARGB(255,255,255,255)));
		...
		PrecalculatedMeshVSConsts replacementVSConsts;
		PrecalculatedMeshVSConsts savedVSConsts;
		...
		r3dPrepareMeshVSConsts( replacementVSConsts, testdummy->GetTransformMatrix(), &unpackScale, &texUnpackScale, r3dRenderer->ViewMatrix, r3dRenderer->ViewProjMatrix );
   ...
   // AntiCheatQuery->Issue( D3DISSUE_END );
	...
	while(true)
	{
		...
		if(hr == S_OK && timeReseted+7 < r3dGetTime())
		{
			if( result && result < 10000 )
			{
				//r3dOutToLog("WallHack Detected!\n");
