...
IMPLEMENT_CLASS(obj_AnimalSpawn, "obj_AnimalSpawn", "Object");
AUTOREGISTER_CLASS(obj_AnimalSpawn);

obj_AnimalSpawn::obj_AnimalSpawn()
{
...
BOOL obj_AnimalSpawn::OnCreate()
{
	lootBoxCfg = g_pWeaponArmory->getLootBoxConfig(lootBoxID);

	/*if((GetPosition() - r3dPoint3D(7048, 41, 6931)).Length() > 5)
	{
		r3dOutToLog("obj_AnimalSpawn skipped\n");
		return FALSE;
	}*/

	r3dOutToLog("obj_AnimalSpawn %p, %d deer's in %.0f meters, delay %.0f, r:%.1f-%.1f\n", 
		this,
...
BOOL obj_AnimalSpawn::OnDestroy()
{
	return parent::OnDestroy();
}

BOOL obj_AnimalSpawn::Update()
{
	...
	if(zombies.size() < maxZombieCount)
	{
		...
		obj_Animal* d = (obj_Animal*)srv_CreateGameObject("obj_Animal", name, pos);
		d->SetNetworkID(gServerLogic.GetFreeNetId());
		d->NetworkLocal = true;
		d->spawnObject  = this;
		d->DetectRadius = u_GetRandom(minDetectionRadius, maxDetectionRadius);
 		d->SetRotationVector(r3dVector(u_GetRandom(0, 360), 0, 0));
...
int obj_AnimalSpawn::CheckForZombieAtPos(const r3dPoint3D& pos, float distSq)
{
	int num = 0;
...
void obj_AnimalSpawn::GenerateNavPoints()
{
	...
	for(pos.x = x1; pos.x < x2; pos.x += NAV_GRID_SIZE)
	{
		...
		{
			r3dPoint3D npos = pos;
			if(! obj_Animal::CheckNavPos(npos))
				continue;
...
int obj_AnimalSpawn::GetFreeNavPointIdx(r3dPoint3D* out_pos, bool mark, float maxDist, const r3dPoint3D* cur_pos)
{
	if(navPoints.size() == 0)
		return -1;
...
void obj_AnimalSpawn::ReleaseNavPoint(int idx)
{
...
bool obj_AnimalSpawn::GetZombieSpawnPosition(r3dPoint3D* out_pos)
{
...
bool obj_AnimalSpawn::GetZombieRandomNavMeshPosition(r3dPoint3D* out_pos)
{
...
#ifdef _DEBUG
	r3dOutToLog("!!! obj_AnimalSpawn_NavFail at %f %f %f.\n", GetPosition().x, GetPosition().y, GetPosition().z);
#endif
...
void obj_AnimalSpawn::ReadSerializedData(pugi::xml_node& node)
{
	...
	GetXMLVal("max_animals", zombieSpawnNode, &mzc);
	maxZombieCount = mzc;
	//GetXMLVal("sleepers_rate", zombieSpawnNode, &sleepersRate);
	GetXMLVal("animals_spawn_delay", zombieSpawnNode, &zombieSpawnDelay);
	...
	GetXMLVal("fastAnimalChance", zombieSpawnNode, &fastZombieChance);
	GetXMLVal("speedVariation", zombieSpawnNode, &speedVariation);
	
	uint32_t numZombies = zombieSpawnNode.attribute("numAnimalSelected").as_uint();
...
void obj_AnimalSpawn::OnZombieKilled(const obj_Animal* d)
{
	for(std::vector<obj_Animal*>::iterator it = zombies.begin(); it != zombies.end(); ++it)
	{
		if(*it == d)
		{
			zombies.erase(it);
	...
	r3dError("Deer %p isn't found on spawn", d);
}
