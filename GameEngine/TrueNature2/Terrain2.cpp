...
r3dTerrain2::DoSave( const char* targetDir, const Shorts& hfShrinkedSamples, int rescaleVertexCountX, int rescaleVertexCountZ )
{
	...
	PackBin( fileName, hfShrinkedSamples, rescaleVertexCountX, rescaleVertexCountZ );
...
int
r3dTerrain2::PackBin( const char* dirName, const Shorts& hfShrinkedSamples, int rescaleVertexCountX, int rescaleVertexCountZ )
{
#if R3D_TERRAIN_V2_GRAPHICS
	FILE * hFile;
	...
	hFile = fopen_for_write( Va( "TerrainData_0.bin", dirName ), "wb");
	r3d_assert( hFile );
	if ( ! hFile )
		return 0;
