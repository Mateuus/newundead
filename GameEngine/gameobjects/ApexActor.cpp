...
ApexActorBase * CreateApexActor(const ApexActorParameters &params, bool makePreview)
{
	...
	if (makePreview)
		return new Private::ApexPreviewActor(params);
	else
		return new Private::ApexActor(params);
}
...
ApexActorBase * ApexActor::ConvertToOpposite()
{
	...
	return new ApexPreviewActor(params);
}
...
NxApexAssetPreview * ApexPreviewActor::CreatePreviewActor()
{
	...
	NxApexAssetPreview *rv = destrAsset->createApexAssetPreview(*previewParams);
	return rv;
}
